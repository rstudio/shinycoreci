...
- [**External package checks**](https://github.com/rstudio/shinycoreci/actions/workflows/external-package-checks.yml): Dispatches package-check workflows in other repos that opt in to `workflow_dispatch`, waits for them to finish, and opens a remediation issue in the target repo if a watched run fails. When a user-scoped Copilot token is configured, the workflow tries to create a Copilot-assigned issue first and falls back to a normal issue if assignment is unavailable. Targets are configured in the workflow matrix; currently `rstudio/reactlog`.
