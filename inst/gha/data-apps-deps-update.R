...
update_shinyverse <- function() {
  infos <- jsonlite::read_json("https://raw.githubusercontent.com/posit-dev-shinycoreci/posit-dev-shinycoreci.r-universe.dev/main/packages.json")

  pkgs <- vapply(infos, `[[`, character(1), "package")
  pkgs_txt <- utils::capture.output(dput(pkgs))
  urls <- vapply(infos, `[[`, character(1), "url")
