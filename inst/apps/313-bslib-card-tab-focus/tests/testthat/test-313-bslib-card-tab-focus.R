...
expect_card_full_screen <- function(app, id) {
  id <- sub("^#", "", id)
  app$wait_for_js(
    'document.body.matches(".bslib-has-full-screen")',
    timeout = 3000
  )
  ...
  expect_equal(
    app$get_js(
      "document.querySelectorAll('.bslib-card[data-full-screen=\"true\"]').length"
    ),
    1
  )
  ...
  expect_equal(
    app$get_js(
      "document.querySelectorAll('#bslib-full-screen-overlay').length"
    ),
    1
  )
  interior_focus <- app$get_js(
    sprintf(
      "document.getElementById('%s').contains(document.activeElement)",
      id
    )
  )
  if (interior_focus) {
...
app_reset_no_full_screen <- function(app) {
  ...
  withr::defer(app$run_js(
    "document.getElementById('neutral-focus-zone').focus()"
  ))
  is_full_screen <- app$get_js(
    "document.body.matches('.bslib-has-full-screen')"
  )
  if (!is_full_screen) {
    return(invisible(app))
  }
  app$click(selector = "#bslib-full-screen-overlay")$wait_for_js(
    '!document.body.matches(".bslib-has-full-screen")'
  )
}
...
test_that("fullscreen card with inputs and interior cards", {
  ...
  expect_display(
    app,
    "none",
    "#card-with-inputs-right .bslib-full-screen-enter"
  )
  # Exit full screen
  app_card_full_screen_exit(app, "escape")
})
...
test_that("fullscreen interior card with inputs (forward tab cycle)", {
  ...
  key_press("Tab")
  Sys.sleep(0.1)
  # After tabbing from the first date input, tabbing behavior is inconsistent.
  # If we're not on the exit button, we're on the second date input, so tab again.
  if (
    !app$get_js(
